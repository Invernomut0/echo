...
> This integration uses **long polling** and lets users chat with ECHO via a Telegram bot.

| Variable | Default | Description |
|----------|---------|-------------|
| `TELEGRAM_ENABLED` | `false` | Enables Telegram bridge at API startup |
...
| `TELEGRAM_POLL_INTERVAL_SECONDS` | `1.0` | Sleep interval between empty polling cycles |
| `TELEGRAM_UPDATE_TIMEOUT_SECONDS` | `30` | Long-poll timeout for `getUpdates` |
| `TELEGRAM_REQUEST_TIMEOUT_SECONDS` | `40.0` | HTTP timeout for Telegram API calls |
| `TELEGRAM_ALLOWED_CHAT_IDS` | `[]` | Optional allow-list of chat IDs (JSON list) |
| `TELEGRAM_HISTORY_TURNS` | `6` | Number of turns cached per Telegram chat |
| `TELEGRAM_MAX_REPLY_CHARS` | `3900` | Max chars per Telegram message chunk |

If `TELEGRAM_ALLOWED_CHAT_IDS` is not empty, only those chats can interact with ECHO.
